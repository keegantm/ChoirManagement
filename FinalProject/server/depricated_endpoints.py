'''
This File Contains the previous implementation of the site's endpoints.
They were replaced with Resful Versions
'''
...
def getRoleAssignmentsByType():
    ...
    try:
        ...
        return jsonify(role_assignments), 200
    except Exception as e:
...
'''
Get Active Voice Parts
    - Get all voice parts of active members
    - Couple members to their voice parts
'''
...
def getActiveVoiceParts():
    try:
        ...
        return jsonify({"error": "An error occurred in getActiveVoiceParts"}), 500

'''
Add Voice Part
    - Adds Voice Part for an input member
'''
...
def add_voice_part():
    try:
        ...
        return jsonify({"message": "Voice part added successfully"}), 201
    except Exception as e:
        print(f"Error in addVoicePart endpoint: {str(e)}")
        app.logger.error(f"Error in addVoicePart endpoint: {str(e)}")
        return jsonify({"error": "An error occurred in addVoicePart"}), 500
...
def deleteVoicePart():
    ...
    try:
        ...
        return jsonify({"error": "An error occurred in deleteVoicePart"}), 500

''' NOTE: Patch may be a better request
...
def updateExistingVoicePart():
    ...
    try:
        ...
        #return the data as JSON
        return jsonify(updated_part_info), 200
    except Exception as e:
        ...
        return jsonify({"error": "An error occurred in updateExistingVoicePart"}), 500
...
def retrievePotentiallyInactiveMembers():
    try:
        ...
        return jsonify(absent_members_list), 200
        ...
        return jsonify({"error": "An error occurred in retrievePotentiallyInactiveMembers"}), 500

''' NOTE: Patch is probably better
Set Inactive Member
...
def setInactiveMember():
    try:
        ...
        return jsonify({"error": "An error occurred in setInactiveMember"}), 500
...
def get_current_budget():
    try:
        ...
        return jsonify({"error": "An error occurred in getCurrentBudget"}), 500
...
def addMember():
    ...
    #for non-unique email
        ...
        return jsonify({"error": "An error occurred in addMember"}), 500

'''
Add Attendance 
    - add an attendance record for a member, on a certain date

    Not fully restful, since could be adding AND updating are options
'''
...
def add_attendance():
    try:
        ...
        app.logger.error(f"Error in addAttendance endpoint: {str(e)}")
        return jsonify({"error": "An error occurred in addAttendance"}), 500


'''
Get enum of absence reasons from DB
'''
...
def getAbsenceReasons():
    ...
    return jsonify(absence_reasons_list), 200

'''
DEPRICATED Get User Permissions
    - Placeholder for generating user permissions.
    - Just gives all True permissions
'''
